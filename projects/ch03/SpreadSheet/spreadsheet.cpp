...
void Spreadsheet::sort(const SpreadsheetCompare &compare)
{
    ...
    for (i = 0; i < range.rowCount(); ++i) {
        QStringList row;
        for (int j = 0; j < range.columnCount(); ++j) row.append(formula(range.topRow() + i, range.leftColumn() + j));
        rows.append(row);
    }
    ...
    for (i = 0; i < range.rowCount(); ++i) {
        for (int j = 0; j < range.columnCount(); ++j) setFormula(range.topRow() + i, range.leftColumn() + j, rows[i][j]);
    }
...
void Spreadsheet::paste()
{
    ...
    for (int i = 0; i < numRows; ++i) {
        ...
        for (int j = 0; j < numColumns; ++j) {
            ...
            if (row < RowCount && column < ColumnCount) setFormula(row, column, columns[j]);
        }
    }
    somethingChanged();
}
...
void Spreadsheet::del()
{
    ...
    if (!items.isEmpty()) {
        foreach (QTableWidgetItem *item, items) delete item;
        somethingChanged();
    }
}
